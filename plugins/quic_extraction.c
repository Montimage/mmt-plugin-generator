 ...
 * 
 * $ gcc -g -o quic_extraction quic_extraction.c -I /opt/mmt/dpi/include -L /opt/mmt/dpi/lib -lmmt_core -ldl -lpcap -lpthread
 * 
 * 
 ...
 * 
 * $ ./quic_extraction -t ftp_trace.pcap > output.xls
 * 
 ...
 */
 int packet_handler(const ipacket_t * ipacket, void * user_args){
	...
	uint32_t *conn_id = (uint32_t *) get_attribute_extracted_data(ipacket, 630, 1);
	uint32_t *seq = (uint32_t *) get_attribute_extracted_data(ipacket, 630, 2);

	if(conn_id !=NULL && seq!=NULL){
		printf("%lu - Connection ID: %u - Sequence: %u\n", ipacket->packet_id,*conn_id,*seq);	
	}
...
int main(int argc, char ** argv){
	...
	register_extraction_attribute(mmt_handler,630,1);
	register_extraction_attribute(mmt_handler,630,2);
