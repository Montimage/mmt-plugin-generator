...
int mmt_check_cpm(ipacket_t * ipacket, unsigned index)
{
  printf("[inf] mmt_check_cpm: %lu - %d\n", ipacket->packet_id, index );
  ...
  if (udp != NULL) {
    ...
    {
     	fprintf(stderr, "[PROTO_CPM] %lu mmt_check_cpm: FOUND CPM!\n", ipacket->packet_id);
      ...
      fprintf(stderr, "[PROTO_CPM] %lu mmt_check_cpm: Not CPM!\n", ipacket->packet_id);
    }
  }
  fprintf(stderr, "[PROTO_CPM] %lu mmt_check_cpm: Not CPM!\n", ipacket->packet_id);
  return 0;
...
int init_cpm_proto_struct() {
	...
	if (protocol_struct != NULL) {
		...
		register_protocol_stack(PROTO_CPM, PROTO_CPM_ALIAS, cpm_stack_classification);
		if (!register_classification_function_with_parent_protocol(PROTO_UDP, mmt_check_cpm, 50)) {
