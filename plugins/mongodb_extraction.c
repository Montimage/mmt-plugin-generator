 ...
 * 
 * $ gcc -g -o mongodb_extraction mongodb_extraction.c -I /opt/mmt/dpi/include -L /opt/mmt/dpi/lib -lmmt_core -ldl -lpcap -lpthread
 * 
 * 
 ...
 * 
 * $ ./mongodb_extraction -t ftp_trace.pcap > output.xls
 * 
 ...
 */
 int packet_handler(const ipacket_t * ipacket, void * user_args){
	...
	uint32_t *length = (uint32_t *) get_attribute_extracted_data(ipacket, MONGODB_PROTO_ID, 1);
	uint32_t *request_id = (uint32_t *) get_attribute_extracted_data(ipacket, MONGODB_PROTO_ID, 2);
	uint32_t *response_to = (uint32_t *) get_attribute_extracted_data(ipacket, MONGODB_PROTO_ID, 3);
	uint32_t *op_code = (uint32_t *) get_attribute_extracted_data(ipacket, MONGODB_PROTO_ID, 4);
...
int main(int argc, char ** argv){
	...
	register_extraction_attribute(mmt_handler,MONGODB_PROTO_ID,1);
	register_extraction_attribute(mmt_handler,MONGODB_PROTO_ID,2);
    register_extraction_attribute(mmt_handler,MONGODB_PROTO_ID,3);
	register_extraction_attribute(mmt_handler,MONGODB_PROTO_ID,4);
